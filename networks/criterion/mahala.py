...
def mahalanobis(u, v, cov_x, use_precision=False, reduction=True):
    ...
    if reduction:
        ...
        return m_loss, num
# def mahalanobis(u, v, inv_cov_x, reduction=True):
#     delta = u - v
#     m_loss = torch.matmul(torch.matmul(delta, inv_cov_x), delta.t())

#     if reduction:
#         return torch.mean(m_loss)
#     else:
#         return m_loss
...
#         cov_diff, _ = cov_v_diff(diff.view(-1, block_size))
...
# def loss_function_mahala(recon_x, x, block_size, cov=None, update_cov=False, use_precision=False, reduction=True):
#     if update_cov == False:
#         loss = mahalanobis(recon_x.view(-1, block_size), x.view(-1, block_size), cov, reduction=reduction)
#         return loss
#     else:
#         diff = x - recon_x
